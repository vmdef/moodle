        ...
        INSERT INTO {block_instances} (
         ...
         WHERE mp.id NOT IN (
            SELECT mpi.id FROM {my_pages} mpi
              JOIN {block_instances} bi ON {$subpagepattern} = mpi.id
...
function upgrade_block_delete_instances(
    ...
    $deleteblockinstances = function (string $instanceselect, array $instanceparams) use ($DB) {
        ...
        EOF;
        $DB->delete_records_subquery('context', 'id', 'cid', $deletesql, $instanceparams);
        ...
        $deletesql = <<<EOF
            SELECT p.id AS pid
              ...
              JOIN {block_instances} bi ON bi.id = bp.blockinstanceid AND p.name IN ({$blockhidden}, {$blockdocked})
             WHERE {$instanceselect}
        EOF;
    ...
    $deleteblockinstances($instanceselect, $instanceparams);

    // Look for any and all instances of the block in customised /my pages.
    $instanceselect = <<<EOF
            blockname = :blockname
        AND pagetypepattern = :pagetypepattern
        AND subpagepattern IN (
            SELECT mp.id FROM {my_pages} mp
             WHERE mp.name = :pagename
