...
class course_gradetopass extends \core\analytics\target\course_enrolments {
     ...
     * If the grade to pass is not set, (max grade - min grade) / 2) will be used as the default grade to pass.
     *
    ...
    protected function get_course_gradetopass ($courseid) {
        if (!isset($this->coursesgradetopass[$courseid])) {
            ...
            if (!grade_floats_different($courseitem->gradepass, 0.0) || is_null($courseitem->gradepass) ||
                ($courseitem->gradepass == $courseitem->grademin)) {
                $courseitem->gradepass = ($courseitem->grademax - $courseitem->grademin) / 2;
            }
    ...
    protected function get_user_grade ($courseitemid, $userid) {
        ...
        if (!isset($this->coursesgrades[$courseitemid])) {
            ...
            $users_grades = $gg->fetch_all(array('itemid' => $courseitemid));

            if ($users_grades) {
                foreach ($users_grades as $ug) {
    ...
    protected function calculate_sample($sampleid, \core_analytics\analysable $course, $starttime = false, $endtime = false) {
        ...
        // Get the user grade.
        $usergrade = $this->get_user_grade($courseitem['id'], $userenrol->userid);
