 ...
 *
 * The purpose of this override is to serve different stylessheets for
 * different domains using one Moodle instance - typically moodle.org/plugins
 * and moodle.org/demo
 *
...
// The domain identifier is fetched from the type which is the stripped filename.
if (preg_match('/all-(\w+)-?(rtl)?$/', $type, $matches)) {
    $domain = $matches[1];
    if (!in_array($domain, $theme->settings->validdomains)) {
        exit(0);
    }
...
function theme_styles_fallback_content($theme) {
    ...
    if (!empty($theme->settings->domain)) {
        $type = 'all-' . $theme->settings->domain . ($theme->get_rtl_mode() ? '-rtl' : '');
