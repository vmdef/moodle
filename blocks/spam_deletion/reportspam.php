...
if ($lib->has_voted($USER->id)) {
    ...
    $discussion = $discussionvault->get_from_id($lib->post->get_discussion_id());
    $forumid = $discussion->get_forum_id();

    // Create akismet spam report.
    $lib->populate_akismet_spam_report();

    // Delete the user's contents and suspend them as a spammer.
    $spamlib = new spammerlib($lib->post->get_author_id());
    $spamlib->set_spammer();

    // Redirect to the original forum.
    redirect(new moodle_url('/mod/forum/view.php', array('f' => $forumid)), 'Spammer deleted');
} else {
    echo $OUTPUT->header();
    ...
    if (!empty($postid) && has_capability('block/spam_deletion:spamdelete', context_system::instance())) {
        ...
        echo $OUTPUT->single_button($deleteurl, get_string('deleteandreporttoakismet', 'block_spam_deletion'), 'post', array('class' => 'p-y-2 mdl-align'));
    }
    echo $OUTPUT->box($lib->content_html(), 'p-y-3');
}
echo $OUTPUT->footer();
