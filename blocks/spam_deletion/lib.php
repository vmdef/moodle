...
class spammerlib {
    ...
    private function delete_user_tags() {
        core_tag_tag::remove_all_item_tags('core', 'user', $this->user->id);
    }
    ...
    public function show_data_count() {
        ...
        $userdata[] = get_string('countmessage', 'block_spam_deletion',
...
abstract class spam_report
{
    ...
    public static function notify_spam($spammerid, $voterid) {
        ...
        $msgsubject = get_string('spamreportmessagetitle', 'block_spam_deletion', $a);
        $msgtext = get_string('spamreportmessage', 'block_spam_deletion', $a);

        $message = new \core\message\message();
        $message->courseid = SITEID;
        $message->component = 'block_spam_deletion';
        $message->name = 'spamreport';
        $message->userfrom = $voter;
        $message->subject = $msgsubject;
        $message->fullmessage = html_to_text($msgtext);
        $message->fullmessageformat = FORMAT_PLAIN;
        $message->fullmessagehtml = $msgtext;
...
class forum_post_spam extends spam_report
{
    ...
    public function __construct($postid) {
        ...
        $this->post = $postvault->get_from_id($postid);
        $this->discussion = $discussionvault->get_from_id($this->post->get_discussion_id());
        $this->forum = $forumvault->get_from_id($this->discussion->get_forum_id());
        $this->course = $this->forum->get_course_record();
        $this->cm = $this->forum->get_course_module_record();
    ...
    public function return_url() {
        ...
        return $urlfactory->get_discussion_view_url_from_discussion($this->discussion);
    }
    ...
    public function has_permission() {
        ...
        return $capabilitymanager->can_view_post($USER, $this->discussion, $this->post);
    }
    ...
    public function register_vote($userid) {
        ...
        $record = new stdClass();
        $record->spammerid = $this->post->get_author_id();
        $record->voterid = $userid;
        $record->weighting = $this->get_vote_weighting($userid);
        $record->postid = $this->post->get_id();
    ...
    public function has_voted($userid) {
        ...
        $params = array('voterid' => $userid, 'postid' => $this->post->get_id());
    ...
    public function content_html() {
        ...
        $s = html_writer::start_tag('div', array('class' => 'path-mod-forum forumpostpreview'));

        $rendererfactory = mod_forum\local\container::get_renderer_factory();
        $postsrenderer = $rendererfactory->get_single_discussion_posts_renderer(FORUM_MODE_THREADED, true);
        $s .= $postsrenderer->render($USER, [$this->forum], [$this->discussion], [$this->post]);
    ...
    public function populate_akismet_spam_report() {
        ...
        $record = array();
        $record['original_id'] = $this->post->get_id();
        $record['is_spam'] = '1';
        ...
        $spammer = $DB->get_record('user', array('id' => $this->post->get_author_id()));
        $spammerprofile = profile_user_record($spammer->id, false);
        ...
        $record['comment_author_url'] = $spammerprofile->url;
        $record['comment_content'] = $this->post->get_subject()."\n".$this->post->get_message();
...
class akismet_table extends table_sql
{
    ...
    public function col_actions($row) {
        ...
        return '<div class="p-1">'.$OUTPUT->single_button($reporturl, 'Report to akismet').'</div>'.
            '<div class="p-1">'.$OUTPUT->single_button($ignoreurl, 'Ignore (not spam)').'</div>';
    }
