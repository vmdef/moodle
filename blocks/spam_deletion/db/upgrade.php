...
function xmldb_block_spam_deletion_upgrade($oldversion) {
    ...
    if ($oldversion < 2015120700) {
        // Convert global CFG settings into the plugin scope settings.
        $settingnames = array(
            'akismet_account_age',
            'akismet_key',
            'badwords',
            'invalidchars_percentage',
            'throttle_duration',
            'throttle_postcount',
        );

        foreach ($settingnames as $settingname) {
            set_config($settingname, get_config('core', 'block_spam_deletion_'.$settingname), 'block_spam_deletion');
            unset_config('block_spam_deletion_'.$settingname);
        }

        upgrade_block_savepoint(true, 2015120700, 'spam_deletion');
    }
