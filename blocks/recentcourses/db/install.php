...
/**
 * Recent courses block installation.
 *
 * @package    block_recentcourses
 * @copyright  2018 Victor Deniz <victor@moodle.com>
 * @author     2018 Ryan Wyllie <ryan@moodle.com>
...
defined('MOODLE_INTERNAL') || die();
 ...
 /**
  * Add the recent courses block to the dashboard for all users by default
  * when it is installed.
  */
function xmldb_block_recentcourses_install() {
    global $DB;

    if ($DB->count_records('block_instances') < 1) {
        // Only add the recentcourses block if it's being installed on an existing site.
    ...
    $page->blocks->add_block('recentcourses', 'content', 0, false, 'my-index', $subpagepattern);
    ...
    // recentcourses block without the admin resetting all of the dashboards.
    //
    // Instead we'll just add the recentcourses block to their dashboards here.
    ...
    foreach ($existingrecords as $existingrecord) {
        ...
        if (isset($seencontexts[$parentcontextid])) {
            ...
            // to add duplicate recentcourses blocks to the same context. This happens
