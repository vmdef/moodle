...
class helper {
    ...
    public static function get_core_settings(): array {
        ...
        $ajaxpaths = [];
        $savefreq = false;
        $ajaxpath = "{$basepath}h5p/ajax.php?contextId={$systemcontext->instanceid}&token=";
        $ajaxpaths['xAPIResult'] = '';
        $ajaxpaths['contentUserData'] = '';

        if (has_capability('moodle/h5p:updatelibraries', $systemcontext)) {
            ...
            if (get_config('core_h5p', 'enable_save_content_state')) {
                $savefreq = get_config('core_h5p', 'content_state_frequency');
            }
        }
