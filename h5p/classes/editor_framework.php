...
class editor_framework implements H5peditorStorage {
    ...
    public static function markFileForCleanup($file, $contentid = null) {
        // TODO Double check if the files with the filearea = 'editor" can be removed
    }
    ...
    public function getLibraries($libraries = null) {
        ...
        // TODO userestrictedlibraries useful? It seems different purpose than updatelibraries
        $superuser = has_capability('moodle/h5p:updatelibraries',
            \context::instance_by_id($syscontext->id));

        if ($libraries !== null) {
            // Get details for the specified libraries only.
            $librarieswithdetails = array();
            foreach ($libraries as $library) {
                // TODO removed fields tutorial_url, restricted, metadatasettings
...
/*            if (!empty($library->tutorial_url)) {
    ...
    public function alterLibraryFiles(&$files, $libraries) {
        // TODO Renderer methods at the moment are not implemented
/*        global $PAGE;
        ...
        $embedtype = 'editor';
        $renderer->render_alter_scripts($files['scripts'], $librarylist, $embedtype);
        $renderer->render_alter_styles($files['styles'], $librarylist, $embedtype);*/
    }
