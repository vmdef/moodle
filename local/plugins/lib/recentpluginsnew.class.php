 ...
 */
class local_plugins_recentplugins_new extends local_plugins_collection_class implements renderable {
    ...
    public function get_rssurl() {
        return local_plugins_helper::get_rss_url('recent_plugins_new', 0);
    }

    public function get_formatted_name() {
        return get_string('mostrecentpluginsnew', 'local_plugins');
    }
    ...
    protected function plugins_alter_query(&$sql, &$params, $unapproved = false, $invisible = false, $alwaysshowown = false) {
        ...
        $sql["WHERE"] .= " AND p.timefirstapproved is not null";
        $sql["ORDER BY"] = "p.timefirstapproved DESC, p.id DESC";
    }
