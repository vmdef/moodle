...
class local_plugins_rss_recent_plugins_updated_feed extends local_plugins_rss_feed {
    public function __construct($id, array $args) {
        parent::__construct($id, $args);
        if ($id) {
            $this->category = local_plugins_helper::get_category($id);
        } else {
            $this->category = new local_plugins_recentplugins_updated();
        }
    }
    ...
    protected function get_most_recent_item_timestamp() {
        global $DB;
        $sql = 'SELECT MAX(p.timelastmodified) AS mostrecent
                  FROM {local_plugins_plugin} p ';
        $sql .= 'Where p.timelastmodified > p.timefirstapproved';
