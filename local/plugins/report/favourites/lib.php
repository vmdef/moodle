...
/**
 * Implements Favourite plugins report
 */
class local_plugins_favourites_report extends local_plugins_report_base {
    ...
    protected function get_report_title() {
        return get_string('report_favourites', 'local_plugins');
    }

    protected function get_report_description() {
        return get_string('report_favouritesdesc', 'local_plugins');
    }

    public function quick_access() {
        // It's better not to put this into the prominent place before we
    ...
    protected function define_columns() {
       $columns = array(
            ...
            'favourited'       => new local_plugins_report_column($this, 'favourited', get_string('favouritedbyx', 'local_plugins'), null, true),
        );
       return $columns;
    }
    ...
    protected function fetch_count() {
        ...
        return $DB->count_records('local_plugins_favourite', array('status' => 1));
    }

    protected function fetch_data() {
        global $USER, $DB;

        $orderby = array('favourited '.$this->sortdir);
        ...
        $sql = "SELECT p.id, p.name, p.shortdescription, c.name AS category $userfields, COUNT(*) AS favourited
        ...
        foreach ($data as $id => $row) {
            ...
            $data[$id]->favourited = $row->favourited;
            unset($data[$id]->id);
