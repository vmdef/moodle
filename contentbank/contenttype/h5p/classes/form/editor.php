...
class editor extends edit_content {
    ...
    protected function definition() {
        ...
        $mform->addElement('hidden', 'contextid', $this->_customdata['contextid']); // automatic sesskey protection
        $this->_form->setType('contextid', PARAM_INT);

        $mform->addElement('hidden', 'plugin', $this->_customdata['plugin']); // automatic sesskey protection
    ...
    public function save_content(\stdClass $data) {
        ...
        // The name of the content is always updated with the title of the H5P content.
        ...
        $h5pcontent = $DB->get_record('h5p', ['id' => $h5pcontentid]);
        $fs = get_file_storage();
        $file = $fs->get_file_by_hash($h5pcontent->pathnamehash);
        // Editing existing content
        if (isset($data->h5pid) && ($data->h5pid = $h5pcontentid)) {
            $cbrecord->id = $file->get_itemid();
            $DB->update_record('contentbank_content', $cbrecord);
        } else {
            ...
            if ($file && $newcontent) {
                ...
                // As itemid changed, pathnamehash has to be updated.
                $updatedfilerecord->pathnamehash = \file_storage::get_pathname_hash($file->get_contextid(), $file->get_component(),
                    $file->get_filearea(), $updatedfilerecord->itemid, $file->get_filepath(), $file->get_filename());
                $DB->update_record('files', $updatedfilerecord);
                $updatedh5precord = new \stdClass();
                $updatedh5precord->id = $h5pcontentid;
                $updatedh5precord->pathnamehash = $updatedfilerecord->pathnamehash;
                $DB->update_record('h5p', $updatedh5precord);
            }
        }
    }
}
