...
class enrol_self_testcase extends advanced_testcase {
     ...
     * @return array
     */
    public function longtimenosee_notifications_provider() {
     ...
     * @param   timestamp   $expirythreshold    How long before expiry are users notified (seconds)
     * @param   timestamp   $customint2         Time of inactivity before unerolling a user (seconds)
     * @param   int         $numnotifications   Expected number of notifications sent
     */
    public function test_longtimenosee_notifications ($expirynotify, $notifyall, $expirythreshold,
        $customint2, $numnotifications) {
        global $DB;
        $this->resetAfterTest();
        ...
        $course1 = $this->getDataGenerator()->create_course(array('fullname' => 'xcourse1', 'startdate' => $coursestartdate));
        ...
        $DB->insert_record('user_lastaccess', array('userid' => $user2->id, 'courseid' => $course1->id, 'timeaccess' => $now -
            DAYSECS * 3));
        ...
        $DB->insert_record('user_lastaccess', array('userid' => $user3->id, 'courseid' => $course1->id, 'timeaccess' => $now
            - DAYSECS * 20));
        ...
        if ($numnotifications && ($customint2 > 0)) {
            ...
            $this->assertContains('You\'ve not visited', $messages[0]->fullmessagehtml);
        }
